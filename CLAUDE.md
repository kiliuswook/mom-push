...
4. **규칙은 문장으로 설명하지 않는다.** 조작·목표·상태는 전부 그림과 공간으로 전한다
   (`Pictogram` / `ActionHint` / `HintPanel` / `GuidePath` / `TargetBoard`).
   실시간 화면에 설명문을 추가하고 싶어지면 그건 연출이 부족하다는 신호다. `docs/GDD.md` 7절 참조.
